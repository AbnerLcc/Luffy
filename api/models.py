...
class Course(models.Model):
    """专题课程 OR 学位课模块"""
    name = models.CharField(max_length=128, unique=True,verbose_name='名称') # Python基础
    course_img = models.CharField(max_length=255)

    sub_category = models.ForeignKey("CourseSubCategory",on_delete=models.CASCADE) #

    course_type_choices = ((0, '付费'), (1, 'VIP专享'), (2, '学位课程'),(4,'免费'))
    ...
    attachment_path = models.CharField(max_length=1024, verbose_name="课件路径", blank=True, null=True)
...
class CourseReview(models.Model):
    ...
    def __str__(self):
        return "%s-%s" % (self.course.name, self.review)
...
class DegreeCourseReview(models.Model):
    ...
    def __str__(self):
        return "%s-%s" % (self.degree_course, self.review)
...
class MyCookie(models.Model):
    str= models.CharField(verbose_name="字符串", max_length=512)
    user=models.ForeignKey(to="UserInfo",on_delete=models.CASCADE,verbose_name='用户')
    currentTime=models.DateField(auto_now=True)
